...
* @rule raw [\s\S]*ttps:\/\/(.{2,}\.isvj(clou)?d\.com(\/[A-Za-z0-9\-\._~:\/\?#\[\]@!$&'\(\)\*\+,;\=]*)?)[\s\S]*
* @rule raw [\s\S]*https:\/\/([\w\.]*[^u]\.jd\.com)[\s\S]*
* @rule raw [\s\S]*export\s+[^=]+=[ ]*"[^"]+"[\s\S]*
* @rule raw [\s\S]*\w+[ ]*=[ ]*"[^"]+"[\s\S]*
* @rule raw http://jd.lsy22.cn/[\d\w]+
* @priority 100
...
const d_api = require("Doraemon_api")
var invalidArr = ["id"];
...
function main() {
	...
	if (IsTarget() || s.isAdmin()) {//仅对监控目标和管理员消息监控
		...
		// 自用特殊处理
		msg = d_api.paramsToUrl(msg);
		
		if (msg.match(/[\s\S]*https:\/\/shop.m.jd.com[\s\S]*/)) {
			// 解析信息中的链接
		...
		// 针对微信线报短连接处理
		if (msg.indexOf('jd.lsy22.cn') != -1) {
			msg = d_api.lsy22ToLongUrl(msg);
		}

		if (msg.indexOf('<title>群聊的聊天记录</title>') != -1) {
			msg = decodeURIComponent(msg);
			// 进行分享的群聊解析
			let urls=[];
			// let values = msg.match(/(?<=encode_url=)[^<]+/g);
			let values = msg.match(/https:\/\/[A-Za-z0-9\-\._~:\/\?#\[\]@!$&'\(\)\*\+,;\=]*/g);
			
			for (let i = 0; i < values.length; i++){
				let url = decodeURIComponent(values[i]);
				if (/.isvj(clou)?d\.com/.test(url)) {
					url = url.replace('&amp;', '&')
					// console.log(url.match(/(\S*)&share/)[1]);
					urls.push(url)
				}
			}
			if (urls.length){
				// console.log(urls);
				Urls_Decode(urls)
				d_api.NotifyTgMonitor(urls);
			}
		} 
		else if (msg.match('<pagepath>')) {
			// console.log('进入小程序解析');
			msg = decodeURIComponent(msg);
			// 如果是单个分享的小程序，进行解析
			// console.log(msg);
			let msg_new = msg.match(/(?<=open_url=)[^\&]+/);
			if (!!msg_new) {
				// URL解码
				// console.log('进入单个分享解析');
				// msg_new = msg_new.toString();
				// msg_new = msg_new.match(/(\S*)&share/)[1]
				// console.log(msg_new);
				Urls_Decode(msg_new)
				d_api.NotifyTgMonitor(msg_new);
			}
		}
...
function Notify(msg) {
	if (db.get("spy_silent_new") != "true" || s.isAdmin()) {
		...
		let from=temp+"--------------------\n\n"
		st.NotifyMainKey("SpyNotify", false, from + msg,db.get("spy_notify_tg_token"))
		st.NotifyMainKey("SpyGroupNotify", true, from + msg,db.get("spy_notify_tg_token"))
	}
}
